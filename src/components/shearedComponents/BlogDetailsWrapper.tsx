...
// components/shearedComponents/BlogDetailsWrapper.tsx
"use client";
import Image from "next/image";
import Link from "next/link";
import React, { useState } from "react";
...
const BlogDetailsWrapper: React.FC<BlogDetailsWrapperProps> = ({ id }) => {
  const { data: blog, isLoading, error } = useGetBlogBySlugQuery(id);
  const [isOpen, setIsOpen] = useState(false);
  const openVideoModal = () => setIsOpen(!isOpen);
  ...
  return (
    <>
      <div className="post-details-wrapper">
          ...
          <div className="post-details-content">
            ...
            <div className="post-details-meta mb-20">
              <ul>
                <li>
                  <Link href="#">
                    <span>By</span> {blog.data.source}
                  </Link>
                </li>
                <li>
                  <Link href="#">
                    <i className="icon-cleander-check"></i> 
                    {moment(blog.data.date).format('MMM DD, YYYY')}
                  </Link>
                </li>
                <li>
                  <Link href="#">
                    <i className="icon-comment"></i> {blog.data.category.title}
                  </Link>
                </li>
              </ul>
            </div>
            ...
            <div className="post-details-tag d-flex flex-wrap gap-10 align-items-center justify-content-between">
              <div className="tagcloud">
                <Link href={`/blog/category/${blog.data.category.slug}`}>
        ...
        <div className="post-avatar">
          <div className="post-avatar-content">
            <h5 className="post-avatar-name">Posted by {blog.data.source}</h5>
            <p className="mb-20">{blog.data.description}</p>
          </div>
        </div>
